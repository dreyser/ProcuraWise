...
`qna` (Fase 17) tiene un único `router.py`, con **dos** `APIRouter` distintos dentro (`vendor_qna_router`, bajo `/vendor-portal/proposals/{proposal_id}/questions`, gateado por `require_agreements_accepted`; `buyer_qna_router`, bajo `/evaluations/{evaluation_id}/questions`, lectura `BUYER_READ_ROLES`/escritura `OWNER_ONLY`) — mismo patrón de separación física de lectura/escritura por rol que `documents`. Grano: **un documento Mongo por pregunta** (`Question`, colección `qna_questions`), con la respuesta actual y su historial embebidos (`current_answer: AnswerVersion | None`, `answer_history: list[AnswerVersion]`) — no una colección `qna_answers` separada, mismo patrón que `Proposal`/`ProposalAnswer` para relaciones 1-a-pocos que nunca se consultan independientemente de su padre. La anonimización de preguntas publicadas es una garantía de tipos, no un filtro en tiempo de ejecución: `PublicQuestionResponse` (lo que ve un proveedor distinto del autor) simplemente no declara ningún campo de identidad, a diferencia de `VendorQuestionResponse` (propia) y `BuyerQuestionResponse` (comprador, identidad real siempre visible). "Notificaciones" en Fase 17 no introdujo ningún mecanismo nuevo — reutilizó el propio `AuditEvent` best-effort ya emitido en cada mutación más un segundo consumidor real de `PollingController`/[ADR 0012](decisions/0012-polling-adaptativo.md) en el frontend (`useQnaPolling.ts`), sin entidad `Notification` ni canal de entrega real; el bounded context `notifications/` completo (entidad, entrega de email real, centro in-app) se construyó en Fase 24 — ver más abajo.

`notifications` (Fase 24 — completo) es el bounded context que estuvo reservado-pero-nunca-construido desde el diseño original (ADR 0001). Grano: **un documento Mongo por `(tenant_id, recipient_membership_id, evento)`** (`Notification`, colección `notifications`), id determinístico (`sha256(tenant_id:event:resource_id:recipient_membership_id)`, no `uuid4()` como `AuditEvent`/`Report`) — esto es lo que permite llamar `NotificationService.notify()` sin envolverlo en un try/except desde un punto de enganche no-best-effort e idempotente como `evaluations.service._finish_publish`: un reintento tras un crash produce el mismo id, `insert_one` lanza `DuplicateKeyError`, tratado como éxito idempotente. Split de entrega: la fila in-app se crea **síncrona y best-effort** (mismo idioma que `AuditEventService.record()`); el envío de email se procesa **asíncrono vía worker** (mismo patrón job que `ai/`/`reports/`), con reintentos de backoff fijo respaldados por un parámetro nuevo, aditivo y retrocompatible en `shared/worker_loop.py::run_worker_loop` (`time_based_tasks: tuple[Callable[[], None], ...] = ()`) — el mismo worker existente absorbe el barrido de reintentos vencidos, sin contenedor/proceso nuevo. Proveedor real `AzureCommunicationServicesEmailProvider` detrás del Protocol `NotificationEmailProvider` (mismo principio de frontera de proveedor externo que `AIProvider`, CLAUDE.md §5.1, extendido aquí a un segundo SDK — ver [ADR 0024](decisions/0024-notificaciones-azure-communication-services.md)); `LoggingNotificationEmailProvider` es el sustituto local/CI. Autorización por **identidad, no por rol** — el único lector/escritor autorizado de una `Notification` es su propio `recipient_membership_id`, resuelto siempre desde el `ActorContext` del request, nunca aceptado como parámetro del cliente (ver threat-model.md). 8 de los 9 eventos de la spec §11 quedan conectados (todos salvo "Fecha próxima/vencida", el único sin punto de enganche síncrono existente — diferido a una generalización futura de `time_based_tasks` hacia un barrido con deduplicación).
...
- **Decisión final** (Fase 22 — completo): bounded context nuevo (`decisions/`), ya anticipado en la lista de subpaquetes desde el diseño original (§3/§9). Sin adjudicación automática por construcción — ninguna transición a `Decision.status="approved"` ocurre sin una llamada HTTP autenticada del `approver` propio de la decisión (nunca el de publicación). `reports/` (Fase 23) consume `DecisionSnapshot` para el tipo `decision_record`, sin recalcular nada retroactivamente; `notifications/` (Fase 24) conecta `approval_requested` tanto para la aprobación de publicación como para la de decisión, con destinatarios independientes.
...
- **Notificaciones reales + centro in-app** (Fase 24 — completo, 8/9 eventos de spec §11, ADR 0024): bounded context `notifications/`, reservado-pero-nunca-construido desde el diseño original (ADR 0001), construido en esta fase (detalle completo en §3). Punto de extensión evidente para una fase futura: el evento restante ("Fecha próxima/vencida") requiere generalizar `time_based_tasks` (ya extendido en esta fase para reintentos de email) hacia un barrido periódico de evaluaciones con deduplicación por umbral cruzado, sin cambiar el modelo de `Notification` en sí.
