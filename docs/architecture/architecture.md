...
- **Versionado de propuestas**: cada `ProposalAnswer` de una nueva versión registra `status: inherited | modified | removed` + `source_proposal_version`; un `Score` pertenece a una versión específica. Ver [ADR 0013](decisions/0013-versionado-propuestas-negociacion.md). `ADR 0013` ya declara que el TCO se recalcula completo por versión, sin mezclar costos entre versiones — el diseño de `CostItem` embebido en `Proposal`/`ProposalSnapshot` (Fase 19) ya es compatible con esa restricción futura sin cambios estructurales.
...
- **`tco/`** (Fase 19 — TCO base; Fase 20 completará el scoring económico): bounded context nuevo, ya anticipado en la lista de subpaquetes desde el diseño original. `TcoService.calculate()` es una función pura (sin dependencias de infraestructura) que implementa la única fórmula de agregación confirmada por el founder para los 3 tipos de costo (`monto(Y) = cantidad × precio_unitario × frecuencia_anual × (1+incremento_anual)^(Y-año_inicio) × (1-descuento)`) — el punto de extensión natural para Fase 20 es consumir `TcoResult.grand_total` de cada proveedor y aplicar la normalización 70% (`menor TCO / TCO proveedor × 100`, spec §7.5) sin tocar este módulo. `Dimension` sigue sin el valor `"economic"` — llega recién en Fase 20.
