...
def _setup(mongo_test_db):
    ...
    service = ProposalService(proposals=proposals, evaluations=evaluations, vendor_orgs=vendor_orgs)
...
def test_concurrent_answer_edits_with_same_expected_version_only_one_wins(mongo_test_db) -> None:
...
def test_submit_with_stale_expected_version_does_not_persist_snapshot(mongo_test_db) -> None:
    tenant, evaluation, proposal, requirement, service, evaluations, proposals, vendor_orgs = (
        _setup(mongo_test_db)
    )
    try:
        ...
        with pytest.raises(StaleVersionError):
            service.submit(tenant.id, proposal.vendor_org_id, proposal.id, 1, "membership-x")
